  ...
  <div class="quiz__form_single">
    <div class="form__flex">
      ...
      <div class="form__group">
        ...
        <p class="form__hint text_small">Выберите из списка</p>
        <el-select
...
/* Стили для загрузки обложки */
.quiz__form_cover_upload {
  display: flex;
  padding: 20px;
  align-items: center;
  flex-direction: column;
...
.quiz__form_cover_upload:hover {
  border-color: var(--color);
  background-color: rgba(0, 0, 0, 0.02);
}
.quiz__form_cover_upload.drag-over {
  border-color: var(--color);
  background-color: rgba(0, 0, 0, 0.05);
  transform: scale(1.01);
}
.quiz__form_cover_upload_content {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 4px;
}
.quiz__form_cover_upload_button {
  color: var(--color);
  transition: color 0.3s linear;
}
.quiz__form_cover_upload:hover .quiz__form_cover_upload_button {
  color: var(--text);
}
.quiz__form_cover_upload_hint {
  max-width: 300px;
  color: var(--text-gray);
}
